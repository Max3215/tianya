...
<div class="tab-content" style="display: block;">
    <dl>
        <dt>用户等级</dt>
        <dd>
            <input name="levelId" type="text" value="<#if user_level??>${user_level.levelId!""}</#if>" ajaxurl="/Verwalter/user/level/check/levelId<#if user_level??>?id=${user_level.id?c}</#if>" class="input txt100" datatype="n1-2" sucmsg=" " errormsg="等级必须是1-2位正整数!">
    ...
    <dl>
        <dt>等级名称</dt>
        <dd>
            <input name="title" type="text" value="<#if user_level??>${user_level.title!""}</#if>" ajaxurl="/Verwalter/user/level/check/title<#if user_level??>?id=${user_level.id?c}</#if>" class="input txt100" datatype="s" sucmsg=" " errormsg="等级名称不能为空!">
    ...
    <dl>
        ...
        <dd>
            ...
            <input name="requiredPoints" type="text" value="<#if user_level??>${user_level.requiredPoints?c}</#if>" class="input txt100" datatype="n" sucmsg="通过" errormsg="积分必须是自然数!">
            <#--<span class="Validform_checktip">*用户累计消费额度到达该值时自动升入该级别</span>-->
            <span class="Validform_checktip">*用户累计积分到达该值时自动升入该级别</span>
        </dd>
    </dl>
    <dl>
        <dt>价格百分比</dt>
...
<div class="page-footer">
    <div class="btn-list">
    	<input name="id" type="text" value="<#if userLevelId??>${userLevelId}</#if>" hidden="hidden">
