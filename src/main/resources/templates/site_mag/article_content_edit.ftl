    ...
    <div class="tab-content" style="display: block;">
	    <dl>
	        ...
	        <dd>
	            <div class="rule-single-select">
	                <select name="categoryId" id="ddlCategoryId"  sucmsg=" " nullmsg="请选择！" class="Validform_error" style="display: none;">
	                	<#if article??>
	                	<#else>
	                    ...
	                    <#if category_list??>
	                        <#list category_list as c>
	                            <option value="${c.id?c!""}" <#if article?? && article.categoryId??  && article.categoryId==c.id>selected="selected"</#if>><#if c.layerCount?? && c.layerCount gt 1><#list 1..(c.layerCount-1) as a>　</#list>├ </#if>${c.title!""}</option>
	                        </#list>
	                    </#if>
	                </select>
	            </div>
        ...
        <dl>
            ...
            <dd>
                ...
                <span class="Validform_checktip">*标题显示颜色</span>
            </dd>
        </dl>
     <script>
        $("#titleColor").spectrum({
