...
<input name="couponId" hidden="hidden" type="text" value='<#if coupon??>${coupon.id?c}</#if>'>
</div>
...
<#else>
 ...
 -->
  <dl>
    <dt>剩余数量</dt>
    <dd>
      <input name="leftNumber" type="text" value="<#if coupon?? && coupon.leftNumber??>${coupon.leftNumber}<#else>0</#if>" class="input small" datatype="n" sucmsg=" ">
      <span class="Validform_checktip">*</span>
    </dd>
  </dl>
  <dl>
