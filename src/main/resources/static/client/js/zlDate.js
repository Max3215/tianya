...
var pickerHtml = {
    ...
    getLeft: function () {
        var left = '<div class="calendar_left pkg_double_month"><p class="date_text">' + obj.year + '年<br>' + obj.month + '月</p><a href="###" title="上一月" id="picker_last" class="pkg_circle_top">上一月</a><a href="###" title="下一月" id="picker_next" class="pkg_circle_bottom ">下一月</a></div>';
        htmlObj.left = left;
    },
    getRight: function () {
        ...
        for (var i = 1; i <= 42; i++) {
            ...
            if ((i - 1) >= week && (i - c) <= days) {
                ...
                var priceStr = "";
                var classStyle = "";
                var childprice = commonUtil.getChildPrice((i - c));
                if (price != -1) {
				...
				if(obj.year==new Date().getFullYear()&&obj.month==new Date().getMonth()+1&&i-c==new Date().getDate()){
					html += '<td  ' + classStyle + ' date="' + obj.year + "-" + obj.month + "-" + (i - c) + '" price="' + price + '" childprice="' + childprice + '"><a><span class="date basefix">今天</span><span class="team basefix" style="display: none;">&nbsp;</span><span class="calendar_price01">' + priceStr + '</span></a></td>';
				}
				else{
                	html += '<td  ' + classStyle + ' date="' + obj.year + "-" + obj.month + "-" + (i - c) + '" price="' + price + '" childprice="' + childprice + '"><a><span class="date basefix">' + (i - c) + '</span><span class="team basefix" style="display: none;">&nbsp;</span><span class="calendar_price01">' + priceStr + '</span></a></td>';
				}
                if (index == 6) {
...
var commonUtil = {
    ...
    getChildPrice:function(day){
        ...
        for (var i = 0; i < obj.priceArr.length; i++) {
            if (obj.priceArr[i].Date == dt) {
                return obj.priceArr[i].childPrice.split('.')[0];
            }
        }
        return -1;
    ...
    chooseClick: function (sender) {
        ...
        var childprice = sender.getAttribute("childprice");
        var el = document.getElementById(elemId);
        if (el != null) {
        //    var choosejson=[{date:date,price:price}];
        //    return choosejson;
		...
		//	alert("价格是：￥"+price);
            ...
            $("#chengrenjia").html(price);
            $("#ertongjia").html(childprice);
            pickerEvent.remove();
        }
    }
}
$(document).bind("click", function (event) {
    ...
    var target = $(e.target);
    if (target.closest("#calendar_choose").length == 0 && target.closest("#calendar").length == 0) {
        pickerEvent.remove();
    }
